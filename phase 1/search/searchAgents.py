        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextNode in self.corners:
                    if nextNode not in sucVCorners:
                        sucVCorners.append(nextNode)
                successor = ((nextNode, sucVCorners), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    while (len(unvisitedCorners) != 0):
        distance, corner = min([(util.manhattanDistance(currentNode, corner), corner) for corner in unvisitedCorners])
        heuristic += distance
        currentNode = corner
